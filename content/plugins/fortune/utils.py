...
from utils.path import res_path, fortune_out_path, fortune_copywriting_path
...
def randomBasemap(_theme: str, _spec_path: Optional[str]) -> str:
    ...
    if isinstance(_spec_path, str):
        _p: Path = res_path / "img"
        p: Path = _p / _spec_path
        return p
    else:
        if _theme == "random":
            __p: Path = res_path / "img"
            # Each dir is a theme
...
def drawing(_theme: str, _spec_path: Optional[str], gid: str, uid: str) -> Path:
    ...
    fontPath = {
        "title": f"{res_path}/font/Mamelon.otf",
        "text": f"{res_path}/font/sakura.ttf",
    }
...
def exportFilePath(originalFilePath: Path, gid: str, uid: str) -> Path:
    ...
    outPath: Path = fortune_out_path / f"{uid}_{gid}.png"
    return outPath
