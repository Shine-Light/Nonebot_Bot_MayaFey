...
import random
import json
from .config import themes_flag_config
from utils.path import fortune_copywriting_path, img_path, font_path, fortune_out_path
...
def get_copywriting() -> Tuple[str, str]:
    ...
    '''
    _p: Path = fortune_copywriting_path
...
def drawing(gid: str, uid: str, theme: str, spec_path: Optional[str] = None) -> Path:
    ...
    fontPath = {
        "title": f"{font_path}/Mamelon.otf",
        "text": f"{font_path}/sakura.ttf",
    }
