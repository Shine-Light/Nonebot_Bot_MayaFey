...
_plugin_meta__ = PluginMetadata(
    ...
    usage="触发方式：指令 + 文字 (部分表情包需要多段文字)\n"
          "发送“/表情包制作”查看表情包列表\n"
          "使用“/表情详情 {表情名}”查看表情帮助" + add_target(60),
    extra={
        "unique_name": "memes",
        "example": "鲁迅说 我没说过这句话\n举牌 aya大佬带带我",
        "generate_type": "group",
        ...
        "permission_special": {
            "memes:block_cmd": "superuser",
            "memes:unblock_cmd": "superuser",
            "memes:block_cmd_gl": "superuser",
            "memes:unblock_cmd_gl": "superuser",
        },
        "unset": False,
        "total_unable": False,
...
help_cmd = on_command("表情包制作", aliases={"头像表情包", "文字表情包"}, block=True, priority=8)
info_cmd = on_command("表情详情", aliases={"表情帮助", "表情示例"}, block=True, priority=8)
block_cmd = on_command("禁用表情", block=True, priority=8)
unblock_cmd = on_command("启用表情", block=True, priority=8)
block_cmd_gl = on_command("全局禁用表情", block=True, priority=8)
unblock_cmd_gl = on_command("全局启用表情", block=True, priority=8)
matcherManager.addMatcher("memes:block_cmd", block_cmd)
matcherManager.addMatcher("memes:unblock_cmd", unblock_cmd)
matcherManager.addMatcher("memes:block_cmd_gl", block_cmd_gl)
matcherManager.addMatcher("memes:unblock_cmd_gl", unblock_cmd_gl)
