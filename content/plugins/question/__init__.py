...
# 添加问答(模糊)
question_vague = on_regex(rf"^{command_start_reg}(模糊问)(.*)答([\s\S]*)", flags=re.RegexFlag.S, priority=13, block=False)
...
# 添加问答(精准)
question_absolute = on_regex(rf"^{command_start_reg}(精准问)(.*)答([\s\S]*)", flags=re.RegexFlag.S, priority=13, block=False)
...
# 添加问答(正则)
question_regular = on_regex(rf"^{command_start_reg}(正则问)(.*)答([\s\S]*)", flags=re.RegexFlag.S, priority=13, block=False)
